...
path_to_JSON_Session_Results = r'C:\Users\Richard\source\repos\Phython_Tests\logs_meeting36\Random_5.json'
path_list = path_to_JSON_Session_Results.split(os.sep)
#change this to your path to JSON meeting profiles -> create a directory that has the json profiles tested on GeniusWeb
path_to_profiles_folder = 'group36meeting'
...
"""New version of numOfRound"""
numOfRounds= json_file['MOPACState']['progress']['rounds']['currentRound']
...
for x in profilesObjects:
  ...
  for utility in range(0,len(list_of_floats)):
      try:
          utilityProductToPlot[utility] *= list_of_floats[utility]
      except:
          continue
  plt.plot(list(range(0,len(list_of_floats))), list_of_floats, linestyle='-', solid_joinstyle='miter', label=x.get_profile_name(), marker = "o")
  count += numOfRounds

#plot product utilities
plt.plot(rounds, utilityProductToPlot, linestyle='-', solid_joinstyle='miter', label="Utility Product", marker = "o")

#plot nash product
nashProduct = utilityProductToPlot
if nashProduct:
    maxValueNash = max(utilityProductToPlot)
    for utility in range(0,int(numOfRounds)):
        nashProduct[utility] = maxValueNash
    plt.plot(rounds, nashProduct, linestyle='-', solid_joinstyle='miter', label="Nash Product", marker="o")

#print(utilityProductToPlot)

"""Saving ProductUtlities"""
name = path_list[-1].split(sep='.')[0]
with open('product_utils/'+name+'.pkl', 'wb') as f:  # Python 3: open(..., 'wb')
    pickle.dump(utilityProductToPlot, f)
