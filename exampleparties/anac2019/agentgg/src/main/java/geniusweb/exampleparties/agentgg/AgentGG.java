 ...
 * anyway). Much javadoc has been added after reverse engineering, to enable the
 * translation.
 */
public class AgentGG extends DefaultParty {
	...
	protected double MAX_IMPORTANCE;
	protected double MIN_IMPORTANCE;
	...
	private void getThreshold(double time) {
		...
		// #1799 HACK not in original code : somethmes above computation goes
		// wildly off bounds.
		if (this.offerLowerRatio > 1)
			this.offerLowerRatio = 0.9999;
...
//    }
	 ...
	 *
	 * @param lowerRatio Generate a lower limit for the random bid in [0,1]
	 * @param upperRatio Generate random bid upper limit can be >1
	 * @return Bid
	 * @throws IOException
	 */
